...
export namespace XLSX {
  ...
  export type ExcelData = Record<
    string,
    Array<any> | Record<string, Array<any>>
  >;
}
...
export const checkTableTitle = async (
  ...
  for (let index = 0; index < titles.length; index++) {
    if (!columns.includes(titles[index])) {
      isVaild = false;
      await ElNotification.error({
        title: "Data error",
        message: titles[index] + " column not found",
      });
    }
  }
  return isVaild;
